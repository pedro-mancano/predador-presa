...
class RK4:
    ...
    def next(self):
        ...
        k2 = self.step * self.func(
            self.time + self.step / 2, np.array(self.state) + k1 / 2
        )
        k3 = self.step * self.func(
            self.time + self.step / 2, np.array(self.state) + k2 / 2
        )
    ...
    def reset(self):
        self.state = np.array(self.initial_conditions, dtype=np.float64)
        self.time = np.float64()
    ...
    def solve_until(self, final_time):
        ...
        while True:
            ...
            t.append(time)
            state_output = np.concatenate(
                (state_output, state[:, :, np.newaxis]), axis=2
            )
